...
func NewController(module core.Module) core.Controller {
	svc := core.Inject[PermissionService](module)
	ctrl := module.NewController("permissions").
		Metadata(swagger.ApiTag("Permission"), swagger.ApiSecurity("bearerAuth")).
		Registry()

	ctrl.
		Pipe(core.BodyParser[UpsertPermissionInput]{}).
		Post("", func(ctx core.Ctx) error {
			contextInfo := core.Execution[middleware.ContextInfo](middleware.APP_CONTEXT, ctx)
			input := core.Execution[UpsertPermissionInput](core.InBody, ctx)
	...
	ctrl.
		Pipe(
			...
			core.BodyParser[UpsertPermissionInput]{},
		).
